...
import stripe
import bcrypt
import pandas as pd  # مكتبة معالجة البيانات وتحويلها لملفات قابلة للتحميل
...
# إعدادات ألعاب اللوتو الأسترالية الرسمية
GAMES = {
...
# ============================================================================
# AUTOMATED LOTTERY FILTERS ENGINE (PRO ENGINE)
# ============================================================================
class LottoEngine:
    @staticmethod
    def generate_single_combination(game, use_filters=False, even_odd_ratio=None, min_sum=None, max_sum=None, locked_numbers=None):
        cfg = GAMES[game]
        pool = list(range(1, cfg["max"] + 1))
        
        valid_locked = []
        if use_filters and locked_numbers:
            valid_locked = sorted(list(set([n for n in locked_numbers if n in pool])))
            if len(valid_locked) > cfg["pick"]:
                valid_locked = valid_locked[:cfg["pick"]]
        
        for _ in range(200):
            if use_filters and valid_locked:
                needed_count = cfg["pick"] - len(valid_locked)
                available_pool = [n for n in pool if n not in valid_locked]
                if len(available_pool) >= needed_count:
                    remaining = random.sample(available_pool, needed_count)
                    numbers = sorted(valid_locked + remaining)
                else:
            ...
            if use_filters:
                if even_odd_ratio:
                    ...
                    odds = cfg["pick"] - evens
                    ratio_str = f"{evens}:{odds}"
                    if ratio_str != even_odd_ratio:
                        continue
                        
                if min_sum and max_sum:
    ...
    @staticmethod
    def generate_batch(quantity, game, use_filters=False, even_odd_ratio=None, min_sum=None, max_sum=None, locked_numbers=None):
        results = []
        for _ in range(quantity):
            combination, bonus = LottoEngine.generate_single_combination(
                game, use_filters, even_odd_ratio, min_sum, max_sum, locked_numbers
            )
...
# التحقق والتحويل التلقائي الكامل فور العودة الناجحة من بوابة Stripe
...
# شاشة التحكم بالملف الشخصي والحساب
...
# دالة مساعدة لتحويل البطاقات إلى نصوص وجداول جاهزة للتحميل والتخزين
def prepare_download_files(tickets, game):
    ...
    for idx, (nums, bns) in enumerate(tickets, 1):
        ...
        csv_data.append({
            ...
            "Game": game,
            "Numbers": nums_str,
            "Powerball/Bonus": bns if bns else "N/A"
        })
        
    return txt_content, pd.DataFrame(csv_data).to_csv(index=False)

# تشغيل الواجهات بناءً على خطة العميل
if st.session_state["user_plan"] == "free":
    st.warning("🔒 Advanced filters (Odd/Even, Golden Sum, Custom Locked Numbers) are locked. Upgrade to Pro for Automated access.")
    ...
    if st.button(f"Generate {ticket_quantity} Standard Ticket(s) (Free)"):
        with st.spinner("Generating tickets..."):
            ...
            # تحضير ملفات التحميل مجاناً للعميل
    ...
    with col1:
        ratio = st.selectbox("Even:Odd Ratio Filter", [None, "3:3", "4:2", "2:4"])
        
        st.write("**📍 Custom Locked Numbers (Optional Filter)**")
        user_numbers_input = st.text_input(
            "Enter numbers to lock/fix (separated by commas, e.g., 5, 12, 23):", 
            placeholder=f"Max {GAMES[game_choice]['pick']} numbers up to {GAMES[game_choice]['max']}"
        )
        
        locked_list = []
        if user_numbers_input:
            try:
                locked_list = [int(n.strip()) for n in user_numbers_input.split(",") if n.strip().isdigit()]
            except ValueError:
                locked_list = []
    ...
    if st.button(f"Generate {ticket_quantity} Advanced Filtered Ticket(s) (PRO)"):
        with st.spinner("Processing advanced filters and generating matrix..."):
