...
class LottoEngine:
    ...
    def get_occupied_decades_count(numbers):
        """دالة ذكية لحساب عدد الخانات العشرية التي تحتوي على أرقام فعلياً"""
        decades = set()
        for n in numbers:
            if 1 <= n <= 9:
                decades.add("single")     # خانة الآحاد (1-9)
            elif 10 <= n <= 19:
                decades.add("tens")       # خانة العشرات (10-19)
            elif 20 <= n <= 29:
                decades.add("twenties")   # خانة العشرينات (20-29)
            elif 30 <= n <= 39:
                decades.add("thirties")   # خانة الثلاثينات (30-39)
            elif 40 <= n <= 49:
                decades.add("forties")    # خانة الأربعينات (40-49)
        return len(decades)
    ...
    def generate_optimized_batch(quantity, game, use_filters=False, even_odd_ratio=None, min_sum=None, max_sum=None, custom_pool=None, empty_slots=None):
        ...
        for _ in range(quantity):
            ...
            for _ in range(500): # رفع محاولات الفحص لضمان دمج الفلتر الجديد المتقدم
                sample = sorted(random.sample(pool, cfg["pick"]))
                
                if use_filters:
                    # 1. فلتر الفردي والزوجي
                    if even_odd_ratio:
                    ...
                    # 2. فلتر المجموع الذهبي
                    if min_sum is not None and max_sum is not None:
                        if not (int(min_sum) <= sum(sample) <= int(max_sum)):
                            continue
                            
                    # 3. [تحديث]: فلتر استبعاد الخانات العشرية المتقدم (Decade Slots Exclusion)
                    if empty_slots is not None and empty_slots > 0:
                        # تحديد كم خانة متاحة في اللعبة بالكامل بناءً على حدها الأقصى
...
# القائمة المفتوحة للجميع لاختيار الاستراتيجية العشوائية أو المخصصة
