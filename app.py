...
if st.session_state["user_plan"] == "free":
    ...
    if st.button(f"Generate {ticket_quantity} Advanced Filtered Ticket(s) (PRO)"):
        with st.spinner("Processing advanced filters and generating matrix..."):
            batch_tickets = LottoEngine.generate_batch(
                ticket_quantity, game_choice, use_filters=True, even_odd_ratio=ratio, 
                min_sum=sum_range, max_sum=sum_range, locked_numbers=locked_list
            )
            for idx, (nums, bns) in enumerate(batch_tickets, 1):
                st.write(f"Filtered Ticket #{idx}: **{nums}**" + (f" | Bonus/Powerball: **{bns}**" if bns else ""))
            txt_file, csv_file = prepare_download_files(batch_tickets, game_choice)
            st.write("---")
