...
with st.sidebar.expander("🎫 Redeem VIP Code"):
...
if st.session_state["user_plan"] == "free":
    ...
    with panel_col1:
        # 🟢 [تم تصحيح السطر الخاطئ هنا وجعله مختصراً على سطر واحد لمنع انكسار الأقواس المربعة]
        ...
        if "Option B" in pool_strategy:
            ...
            raw_pool_input = st.text_area(
                "Inject mathematical pool criteria separated explicitly by commas (e.g., 2, 8, 14, 22, 33, 41):",
                placeholder=f"Enter integers from 1 up to {GAMES[game_choice]['max']}"
            )
            if raw_pool_input:
                try:
                    extracted_custom_pool = sorted(list(set([int(item.strip()) for item in raw_pool_input.split(",") if item.strip().isdigit() and 1 <= int(item.strip()) <= GAMES[game_choice]['max']])))
