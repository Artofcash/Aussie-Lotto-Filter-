...
if st.session_state["user_plan"] == "free":
    ...
    with col1:
        ...
        if "Option B" in pool_option:
            ...
            if user_pool_input:
                try:
                    parsed_pool = [int(n.strip()) for n in user_pool_input.split(",") if n.strip().isdigit()]
    ...
    if st.button(f"Generate {ticket_quantity} Advanced Filtered Ticket(s) (PRO)"):
        if "Option B" in pool_option and len(parsed_pool) < GAMES[game_choice]['pick']:
            st.error("Cannot generate tickets. Please provide enough custom numbers or switch to Option A.")
        else:
            with st.spinner("Processing advanced filters and generating matrix..."):
                batch_tickets = LottoEngine.generate_batch(
                    ...
                    min_sum=sum_range, max_sum=sum_range, custom_pool=parsed_pool
                )
