            ...
            for _ in range(500):
                ...
                if use_filters:
                    ...
                    if empty_slots is not None and empty_slots > 0:
                        total_slots = LottoEngine.get_available_decades_count(cfg["max"])
                        occupied_slots = LottoEngine.get_occupied_decades_count(sample)
                        actual_empty = total_slots - occupied_slots
                        if actual_empty != empty_slots:
                            continue
                combination = sample
                matched = True
                break
